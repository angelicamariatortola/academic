...
# Criando a tabela com as funções e suas descrições
funcoes <- data.frame(
  ...
  Descricao = c(
    ...
    "Indices dos elementos que atendem a uma condição",
    "Valores únicos de um vetor",
    "Frequência de cada elemento do vetor",
  ...
  Exemplo = c(
    ...
    "sort(c(5, 1, 3)) → 1 3 5",
    "which(c(2, 5, 8) > 4) → 2 3",
    "unique(c(1, 1, 2, 3, 3)) → 1 2 3",
    "table(c('A', 'B', 'A')) → A:2 B:1",
    "c(2, 3, 4) %in% c(3, 4, 5) → FALSE TRUE TRUE"
  )
)

# Gerando a tabela formatada
kable(funcoes, caption = "Resumo das Funções para Vetores no R", align = "c")
```
