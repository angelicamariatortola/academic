...
nome <- "https://angelicamariatortola.github.io/academic/Figure7_results.txt" 
simu_nvar_data <- read.table(nome, header = T)

df <- data.frame(
  var = as.factor(simu_nvar_data$nvar), 
  size = (simu_nvar_data$size)^2, 
  tempo = simu_nvar_data$elapsed/3600) # em horas
names(df) <- c("N. variables", "Size", "Time")
